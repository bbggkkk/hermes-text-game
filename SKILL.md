...
게임 로직과 세이브/설정 관리는 [game_engine.py](file:///home/hana/sources/@hermes/hermes-text-game/scripts/game_engine.py)를 사용합니다.
...
1.  **게임 초기화 (Init)**:
    ...
    ```bash
    python3 scripts/game_engine.py --init --config-data '<Config_JSON_문자열>' --state-data '{}'
    ```
...
2.  **행동 진행 (Command)**:
    ...
    ```bash
    python3 scripts/game_engine.py --command "<mapped_command>" --config-data '<Config_JSON_문자열>' --state-data '<직전_턴의_game_state_JSON_문자열>'
    ```
